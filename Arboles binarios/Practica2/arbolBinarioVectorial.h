...
template <typename T> class Abin{
	...
	public:
		...
		explicit Abin(std::size_t maxNodos);

		// Explicit es para que no se pueda declarar una instancia con Abin a;
		// Que sería equivalente a Abin a(23);
...
template <typename T>
inline Abin<T>::Abin(std::size_t maxNodos):
...
template <typename T>
void Abin<T>::insertarHijoIzqdoB(Abin::nodo n, const T& e){
...
template <typename T>
void Abin<T>::insertarHijoDrchoB(Abin::nodo n, const T& e){
	assert(n >= 0 && n < numNodos);		// nodo valido
...
template <typename T>
void Abin<T>::eliminarHijoIzqdoB(Abin::nodo n){
...
template <typename T>
void Abin<T>::eliminarHijoDrchoB(Abin::nodo n){
...
template <typename T>
inline typename Abin<T>::nodo Abin<T>:: padreB(Abin::nodo n) const{
	assert(n >= 0 && n < numNodos);
	return nodos[n].padre;
}

template <typename T>
inline typename Abin<T>::nodo Abin<T>::hijoIzqdoB(Abin::nodo n) const{
	assert(n >= 0 && n < numNodos);
	return nodos[n].izquierdo;
}

template <typename T>
inline typename Abin<T>::nodo Abin<T>::hijoDrchoB(Abin::nodo n)const{
	return nodos[n].derecho;
}


template <typename T>
Abin<T>::Abin(const Abin<T>& arbol):
